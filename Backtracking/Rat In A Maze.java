...
public static boolean wayOut(int[][] maze, int i, int j, int[][] path) {
		//check if i, j cell is valid or not
		int n = maze.length;
		if(i<0 || i>=n || j<0 || j>=n || maze[i][j] == 0 || path[i][j] == 1) {
			return false;
		}
		//Include the current cell in current path
		path[i][j] = 1;
		//if cell reached is destination cell
		if(i == n-1 || j == n-1) {
			return true;
		}
		...
		/*Top*/
		if(wayOut(maze, i-1, j, path)) {return true;}
		/*Right*/
		if(wayOut(maze, i, j+1, path)) {return true;}
		/*Down*/
		if(wayOut(maze, i+1, j, path)) {return true;}
		/*Left*/
		if(wayOut(maze, i, j-1, path)) {return true;}
